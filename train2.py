...
class SCPFeaturesExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Box):
        ...
        # 1. Capa de "Razonamiento Espacial": Mezcla sensores antes de la conv
        # Obliga a la red a relacionar los sensores entre sí
        self.spatial_mlp = nn.Sequential(
            nn.Linear(self.frame_dim, 128),
            nn.ReLU()
        )

        # 2. Convolución para capturar dinámicas temporales
        # Ahora in_channels es 128, lo que hace el filtrado mucho más rico
        ...
        with torch.no_grad():
            # El dummy input debe coincidir con la salida de spatial_mlp
            dummy_input = torch.zeros(1, 128, self.num_frames)
            self.out_dim = self.temporal_conv(dummy_input).shape[1]
    ...
    def forward(self, observations):
        ...
        x = observations.view(batch_size, self.num_frames, self.frame_dim) # (B, 5, 259)
        
        # 2. Aplicar el MLP espacial a cada frame de forma independiente
        # x shape pasa a (B, 5, 128)
        x = self.spatial_mlp(x)
        
        # 3. Permutar para Conv1d (B, C, L) -> (B, 128, 5)
        x = x.permute(0, 2, 1)
